...
.item{
    ...
    margin-top:-50px;
          ...
          <?php
            ...
            //$result = mysql_query("UPDATE `controls` SET `room` = '$room', `name` = '$name', `outputs` = '$output', `type` = '$type' WHERE `id` = '$idcko'");
            $result = mysql_query("INSERT INTO controls(room, name, outputs, type) VALUES('$room','$name','$output','$type')");        
            if(!$result) {
                 $chyba = mysql_error();
