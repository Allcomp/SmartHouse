<?php
    ...
    $classLoader->run();
    $database = new Database(new DatabaseConfig("localhost", "root", "alvasek", "smart_control"));
?>
...
<html>
    <head>
        ...
        <script>
            ...
            function requestUpdateOutputs() {
                ...
                var roomControlItemElems = $("div.roomControls div.item").toArray();

                for(var i = 0; i < roomControlItemElems.length; i++) {
                    var divControls = $(roomControlItemElems[i]).find("div.controls").toArray()[0];
                    var input = $(divControls).find("input").toArray()[0];
                ...
                script.src = 'http://10.0.0.156:81/updateOutputs/states/' + outputString;
                document.getElementsByTagName('head')[0].appendChild(script);
            }

            function requestOutputChange(output, value) {
                var script = document.createElement('script');
                script.src = 'http://10.0.0.156:81/voidCatch/output/' + output + "/" + value;
                document.getElementsByTagName('head')[0].appendChild(script);
            }
            ...
            function executeMacro(controls) {
                ...
                var script = document.createElement('script');
                script.src = 'http://10.0.0.156:81/voidCatch/outputs/' + cmdString;
                document.getElementsByTagName('head')[0].appendChild(script);
            }
    ...
    <body onload="">
        ...
        <div class="mainMenu">
            ...
            <div class="item" onclick="notification('Není důvod něco nastavovat, když všechno běží, jak má ;)');">
                <div class="img"><img src="res/icons/Settings%203%20Filled-100.png" /></div>
                <div class="label">Nastavení</div>
            </div>
            <div class="item" onclick="notification('A kdo by vám ji dal o.O ?');">
